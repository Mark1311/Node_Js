...
Node.js me Streams ka use bohot bade data (jaise videos, badi files, ya network packets) ko chhote-chhote hisson (chunks) me process karne ke liye kiya jata hai. Isse faida ye hota hai ki poori file ko memory (RAM) me load nahi karna padta, jisse aapka application kam memory me bhi fast chalta hai.
...
* Real-life analogy: Ek behta hua jharna (waterfall) jahan se aap paani le rahe hain.
...
* Real-life analogy: Ek khali balti jise aap paani se bhar rahe hain.
...
* Real-life analogy: Ek Phone Call. Aap ek hi samay par bol bhi sakte hain aur sun bhi sakte hain.
...
* Real-life analogy: Ek paani ka filter (Purifier). Ganda paani andar jata hai, filter hota hai, aur saaf paani bahar aata hai.
...
```
### pipe() Method Kya Hai aur Iska Kya Kaam Hai?
Maan lijiye aapko ek badi video file ko read karna hai aur use dusri jagah write (copy) karna hai. Agar aap manually karenge, to aapko Readable stream ke data events ko sunna padega, fir chunk ko Writable stream me write karna padega, aur backpressure (jab data read fast ho aur write slow) ko bhi handle karna padega.
```js
const fs = require('fs');

// 1. Readable Stream banayi (Yahan se data aayega)
const readableStream = fs.createReadStream('badi_file.mp4');

// 2. Writable Stream banayi (Yahan data jayega)
const writableStream = fs.createWriteStream('file_ki_copy.mp4');

// 3. pipe() ka use karke dono ko jod diya
readableStream.pipe(writableStream);

console.log("File successfully stream ho rahi hai!");
```
...
# Event Loop
Node.js ka Event Loop hi wo secret sauce hai jo ise asynchronous aur non-blocking banata hai. Jab aapka JavaScript code execute hota hai, to wo alag-alag operations (jaise file padhna, network request, ya timers) ko alag-alag "phases" me handle karta hai.

### Event Loop ke Phases
Event Loop ek cycle (ya loop) me chalta hai, aur har cycle me 6 main phases hote hain. Har phase ke paas apna ek FIFO (First-In-First-Out) queue hota hai jisme callback functions wait kar rahe hote hain.

- Timers Phase: Yahan setTimeout() aur setInterval() ke callbacks execute hote hain, jab unka threshold time poora ho jata hai.

- Pending Callbacks Phase: Kuch system operations ke callbacks yahan execute hote hain, jaise TCP errors (e.g., agar ECONNREFUSED mila).

- Idle, Prepare Phase: Ye Node.js ke internally use ke liye hota hai, hamare liye iska zyada matlab nahi hai.

- Poll Phase: Ye sabse important phase hai. Yahan naye I/O events (jaise file read/write, incoming network requests) ko dhoonda aur handle kiya jata hai. Agar koi timer ready nahi hai, to event loop yahan thoda wait bhi kar sakta hai.

- Check Phase: Yahan sirf aur sirf setImmediate() ke callbacks execute hote hain.

- Close Callbacks Phase: Agar koi socket ya handle achanak close ho jaye (e.g., socket.on('close', ...)), to wo yahan execute hota hai.

# diff setImmediate() vs setTimeout()

### 1. Jab I/O Cycle ke andar call kiya jaye (Recommended Scenario)
Agar aap kisi file handling ya network request ke callback ke andar dono ko call kar rahe hain, to setImmediate() hamesha pehle chalega.

Kyun? Kyunki I/O operations Poll Phase me handle hote hain. Poll phase ke agla phase hi Check Phase hota hai (jahan setImmediate chalta hai), jabki Timers phase tak pahunchne ke liye loop ko poora ghoom kar aana padega.
```js
const fs = require('fs');
...
// Output (Hamesha same rahega):
// 1. setImmediate (Check Phase)
// 2. setTimeout (Timer Phase)
```

### 2. Jab Main Module (Global Scope) me call kiya jaye
Agar aap dono ko normal script ke upar bina kisi I/O ke likhte hain, to unka order unpredictable (un-ordered) hota hai. Ye system ki performance aur CPU par depend karta hai.
```js
setTimeout(() => {
...
// Output unpredictable hai:
// Kabhi setTimeout pehle chalega, to kabhi setImmediate.
```

# What is Libuv?
Libuv ek multi-platform C library hai jise mukhya roop se Node.js ke liye hi develop kiya gaya tha (halaanki ab ise doosre projects jaise Julia aur Neovim bhi use karte hain).

JavaScript engine (V8) akele asynchronous kaam (jaise file padhna, database se connect hona, ya network request bhejna) nahi kar sakta. V8 sirf JavaScript code ko samajhta hai aur use execute karta hai (jo ki single-threaded hota hai). Libuv wo system hai jo Node.js ko piche se "superpowers" deta hai taaki wo asynchronous aur non-blocking operations handle kar sake.

### Used in Node
Jab aap Node.js me koi asynchronous task perform karte hain (jaise fs.readFile), to Node.js us kaam ko Libuv ko saump (handover kar) deta hai. Libuv us kaam ko background me do tarike se handle karta hai:

### Libuv ke Main Responsibilities

- Event Loop provide karna: Jo alag-alag phases me chalte hue callbacks ko sahi time par execute karta hai (jiske baare me humne pehle baat ki thi).

- Thread Pool Manage karna: Heavy synchronous tasks ko background me chalane ke liye.

- Cross-Platform Consistency: Windows, Linux, aur macOS ke standard inputs/outputs aur file systems alag hote hain. Libuv ek aisa layer (wrapper) banata hai jisse aapka Node.js code har OS par bina badle sahi tarike se chalta hai.

> Pro Tip: Agar aapka application bohot zyada crypto ya file handling ka kaam karta hai, to aap Libuv ke default 4 threads ko badha bhi sakte hain. Iske liye aapko apne terminal ya environment file me process.env.UV_THREADPOOL_SIZE = 8; set karna hota hai.

# Node.js single-threaded hai, toh ye concurrency kaise handle karta hai?
Ye bilkul sahi baat hai ki Node.js single-threaded hai, yaani iske paas JavaScript code ko chalane ke liye sirf ek hi Main Thread hota hai.

Lekin fir bhi Node.js ek hi samay par hazaron (even lakhon) concurrent connections ko bina ruke sambhal leta hai. Iska secret hai: Non-blocking I/O aur Asynchronous Architecture (jise Libuv aur Event Loop chalate hain).

### Node.js Concurrency Kaise Achieve Karta Hai?
Jab aap Node.js me koi aisi request handle karte hain jisme time lagta hai (jaise Database se data nikalna ya External API call karna), to ye steps hote hain:

- Request Reception: Request Main Thread par aati hai.

- Offloading (Kaam Saunpna): Agar wo kaam I/O heavy hai (jaise database query), to Main Thread us kaam ko khud karne ke bajay Libuv aur Operating System (OS) ko de deta hai.

- Main Thread Free: Kaam transfer karte hi Main Thread turant agli request ya agla code execute karne ke liye free ho jata hai. Wo block nahi hota.

- Background Execution: OS ya Libuv ka Thread Pool us heavy kaam ko background me chalate hain.

- Callback Queue & Event Loop: Jab background me kaam poora ho jata hai, to uska callback function ek queue me chala jata hai. Event Loop lagatar check karta rehta hai ki agar Main Thread free hai, to wo queue se callback ko uthakar Main Thread par execute kar deta hai.

### Node.js Kahan Fail Hota Hai?
Node.js I/O bound kaamo me to king hai, lekin agar aapne isme koi CPU bound kaam kar diya (jaise 1 se 10 billion tak ka loop chala diya, ya heavy image processing ki), to Main Thread wahi fass (block) jayega. Jab tak wo loop khatam nahi hoga, tab tak baki kisi bhi customer ki request handle nahi ho payegi.

🛠️ Note: CPU intensive tasks ko handle karne ke liye modern Node.js me Worker Threads ka feature diya gaya hai, jisse aap zaroorat padne par naye threads bana sakte hain.

| Feature | Multi-threaded (e.g., Java, Apache) | Node.js (Single-threaded) |
| :--- | :--- | :--- |
| **Thread Management** | Har request ke liye naya thread banta hai ya pool se liya jata hai. | Saari requests ek hi main thread par aati hain. |
| **Memory Consumption** | High (Kyunki har thread memory leta hai). | Low (Ek hi thread hone ki wajah se resource bachte hain). |
| **I/O Operations** | Thread tab tak block rehta hai jab tak I/O complete na ho. | Non-blocking. I/O background me hota hai. |
| **Best Suited For** | CPU-intensive tasks (Jaise Video editing, Complex Math, Image Processing). | I/O-intensive tasks (Jaise Chat applications, Streaming, REST APIs). |

# Worker Threads aur Cluster Module me kya difference hai? Dono ko kab use karna chahiye?

| Feature | Worker Threads (`worker_threads`) | Cluster Module (`cluster`) |
| :--- | :--- | :--- |
| **Basic Concept** | Ek hi Process ke andar naye **Threads** banata hai. | Ek hi Application ke naye **Processes** (Workers) banata hai. |
| **Memory Sharing** | Ye threads aapas me **Memory Share kar sakte hain** (`SharedArrayBuffer` ke jariye). Isliye ye bohot fast hote hain. | Har process ki **apni alag memory hoti hai**. Ye aapas me memory share nahi kar sakte (IPC ke jariye data transfer hota hai). |
| **Port Sharing** | Alag-alag threads same network port (e.g., 3000) ko direct share nahi karte. | Saare worker processes **same port (e.g., 3000) ko share karte hain**. Cluster module load balancing khud sambhalta hai. |
| **Overhead / Cost** | Lightweight hote hain. Naya thread banane me memory aur CPU kam lagta hai. | Heavyweight hote hain. Naya process banane me zyada memory aur resources lagte hain (har process ka apna V8 instance hota hai). |
| **Crash Impact** | Agar ek Worker Thread crash ho jaye, to wo pure Main Process ko crash kar sakta hai agar proper error handling na ho. | Agar ek Worker Process crash ho jaye, to baaki processes chalte rehte hain. Aap crash hue process ko turant restart kar sakte hain. |
| **Primary Use Case** | **CPU-intensive tasks** (Calculations, Image/Video processing, Cryptography). | **Scaling Network Traffic** (Incoming HTTP requests ko multi-core CPU par divide karna). |

### Worker Threads Kab Use Karein?
Jab aapka application kisi ek hi request me bohot bada ya bhari calculation kar raha ho, jisse Node.js ka main thread block ho raha ho.
- Kise bade image ya video ko resize/compress karna.
- PDF generate karna ya heavy Excel sheets process karna.
- AI/Machine learning ke algorithms ya complex mathematical calculations chalana.
- Data encryption/decryption (Bcrypt, Crypto).
### Cluster Module Kab Use Karein?
Jab aapke paas ek standard web server (API ya Website) ho aur aap chahte hain ki wo ek hi samay par hazaron naye users (traffic) ko jhel sake, bina crash hue.
- Ek REST API server jahan bohot zyada concurrent HTTP requests aa rahi hain.
- Ek Chat server ya E-commerce website jise multi-core CPU ki poori takat ka use karke scale karna hai.
- Application me high availability (bina ruke chalte rehna) achieve karne ke liye, taaki agar ek process crash bhi ho to doosra sambhal le.

Pro Tip: Real-world me, bade production applications me in dono ka sath me bhi use kiya jata hai. Cluster Module ka use karke server ko saare CPU cores par scale kiya jata hai taaki traffic handle ho sake, aur unhi ke andar agar koi heavy task aaye, to use Worker Threads ko de diya jata hai.

# Agar ek Promise reject ho jaye aur hum use catch na karein, toh kya hoga? Isse kaise handle karte hain?
Pehle ke Node.js versions me ispar sirf ek warning aati thi, lekin modern Node.js me agar aap koi rejection handle nahi karte, toh aapka poora Node.js process (server) turant crash ho jata hai.

### Unhandled Rejection hone par kya hota hai?
Jab koi promise reject hota hai aur use handle nahi kiya jata, toh Node.js ek error throw karta hai: UnhandledPromiseRejection.

Modern Node.js architecture me, ye error seedhe process ko terminate (band) kar deta hai aur exit code 1 ke sath crash ho jata hai.

Kyun? Kyunki unhandled rejection ka matlab hai ki aapke application me koi aisi galti ya error aaya hai jiske baare me aapko pata nahi tha (jaise database down hona). Aise me application ko chalte rehne dena dangerous ho sakta hai kyunki wo unpredictable state me chala jata hai.

> Isse Kaise Handle Karte Hain? (3 Tarike)
- 1. .catch() ka use karke (Promise Chains me)
- 2. try...catch ka use karke (Async/Await me)
- 3. Global Level par Handle Karna (Safety Net)

⚠️ Zaroori Warning: Global unhandledRejection listener lagane se aapka server crash hone se toh bach jayega, lekin ise har jagah local catch lagane ka substitute (vikalp) mat banaiye. Iska use sirf un errors ko pakadne ke liye karein jo galti se chhoot gaye hon, aur iske andar error ko achhe se log zarur karein.

# Node.js me Memory Leak ko kaise detect aur debug kiya jata hai?
Node.js me Memory Leak tab hota hai jab JavaScript ke kuch objects ya variables ab kaam ke nahi rahe, lekin unka reference (link) abhi bhi memory me bana hua hai. Is wajah se Garbage Collector (GC) unhe delete nahi kar pata, aur dheere-dheere aapka server poori RAM consume karke crash (FATAL ERROR: Ineffective mark-compacts near heap limit Allocation failed - JavaScript heap out of memory) ho jata hai.

#### Memory Leak ko Detect Kaise Karein?
Pehle aapko ye pata hona chahiye ki leak ho bhi raha hai ya nahi. Iske liye ye tarike use kiye jate hain:
 > A. Process Metrics Check Karna
 ```js
 console.log(process.memoryUsage());
/* Output kuch aisa dikhega:
{
  rss: 49356800,     // Total memory allocated for the process
  heapTotal: 11264000, // Total size of the allocated heap
  heapUsed: 6511928,   // Actually used memory (Ispar nazar rakhni hai)
  external: 1045790
}
*/
```
`Agar heapUsed lagatar badhta ja raha hai aur kabhi kam nahi hota (hathiyar daalne ke baad bhi), toh memory leak pakka hai.`

# Blocking vs Non-blocking

- Blocking: Jab tak koi kaam (jaise file read karna) poora nahi ho jata, tab tak aapka code aage nahi badhega. Main thread wahi ruk kar wait karega.

- Non-blocking: Jab aap kisi kaam ki request karte hain, to system turant reply kar deta hai. Agar data ready nahi hai, to wo "Error: Resource Temporarily Unavailable" ya "Pending" return kar dega, lekin aapke main thread ko rokega nahi. Aapka code aage badh sakta hai.

# Synchronous vs Asynchronous
- Synchronous: Kaam ek sequence (line-by-line) me hota hai. Ek task khatam hoga, tabhi doosra shuru hoga. Result turant milta hai.

- Asynchronous: Aapne kaam shuru karke chhod diya (ya kisi aur ko de diya). Wo kaam background me chalta rahega, aur aapka code agli lines execute karne lagega. Jab background wala kaam poora ho jayega, to aapko ek notification (Callback, Promise, ya Event) ke jariye bataya jayega.

## Real-Life Example:
> Scenario A: Synchronous + Blocking Approach
- Aap Cashier ke paas gaye aur Coffee order ki.
- Cashier khud kitchen me gaya, coffee banane laga, aur aap counter par khade rahe.
- Jab tak coffee ban nahi gayi, Cashier ne kisi aur customer ka order nahi liya (Blocking).
- Line me lage baaki sabhi log wait kar rahe hain.

## Scenario B: Asynchronous + Non-blocking Approach (Node.js Way)
- Aap Cashier ke paas gaye aur Coffee order ki.
- Cashier ne aapka order liya, kitchen me Barista ko bhej diya, aur aapko ek Token / Receipt de di (Non-blocking - Cashier free ho gaya).
- Cashier ne aapko wahan khade rakhne ke bajay side me wait karne ko kaha, aur turant line me lage agle customer ka order lene laga.
- Background me Barista coffee bana raha hai (Asynchronous operation).
- Jab aapki coffee taiyar ho jati hai, to Barista bell bajata hai ya aapka Token Number call karta hai (Callback Function). Aap jate hain aur apni coffee utha lete hain.

# Event Loop kya hai? Ye Node.js me kaise kaam karta hai?
Event Loop Node.js ka wo sabse mahatvapurna hissa (runtime engine) hai jo ise single-threaded hone ke baad bhi asynchronous aur non-blocking banata hai.

Agar simple shabdon me kahein, toh JavaScript engine (V8) ek samay me sirf ek hi kaam kar sakta hai kyunki wo single-threaded hai. Lekin real-world applications me hume file read karni hoti hai, database queries chalani hoti hain, aur network requests handle karni hoti hain—jinme time lagta hai. Event Loop ka kaam yahi hai ki wo in saare background tasks ko manage kare aur jaise hi koi kaam poora ho, uske callback function ko sahi samay par execute kare.

### Event Loop ke Phases
Event Loop ek cycle (loop) me ghoomta hai aur har cycle me 6 main phases se guzarta hai. Har phase ke paas apna ek queue hota hai:

- Timers Phase: Yahan setTimeout() aur setInterval() ke callbacks execute hote hain jinka time poora ho chuka hai.
- Pending Callbacks Phase: Kuch system-level operations ke callbacks (jaise TCP error types) yahan handle hote hain.
- Idle, Prepare Phase: Ye Node.js ke internal use ke liye hota hai.
- Poll Phase: Ye sabse critical phase hai. Yahan naye I/O events (incoming network connections, data reading) ko dhoonda aur handle kiya jata hai. Agar koi kaam nahi hai, toh loop yahan thodi der rukta hai.
- Check Phase: Yahan sirf setImmediate() ke callbacks chalte hain.
- Close Callbacks Phase: Agar koi socket ya handle achanak close ho jaye (e.g., socket.on('close')), toh uske callbacks yahan chalte hain.

### Event Loop Kaise Kaam Karta Hai? (The Workflow)
- Call Stack: Jab aapka JavaScript code execute hota hai, toh saare functions line-by-line Call Stack me jaate hain aur execute hokar bahar nikal jaate hain.
- Offloading to Libuv: Agar Call Stack me koi asynchronous operation aata hai (jaise fs.readFile ya setTimeout), toh Node.js use stack se hata kar background me Libuv (C++ library) aur OS Kernel ko saump deta hai. Main thread aage ke code par badh jata hai.
- Task Completion: Background me jab wo file read ya timer ka kaam poora ho jata hai, toh uska callback function Callback Queue (ya Task Queue) me bhej diya jata hai.
- The Event Loop's Job: Event Loop ka sirf ek hi lagatar chalne wala kaam hai—wo Call Stack aur Callback Queue dono par nazar rakhta hai.
  - Rule: Jab Call Stack bilkul khali (empty) ho jata hai, tab Event Loop Callback Queue se pehla function uthata hai aur use chalane ke liye Call Stack me daal deta hai.

# package.json aur package-lock.json me kya farq hai?
> package.json (The Manifesto)
- Ye aapke project ki ek high-level summary ya manifesto hoti hai. Isme project ka naam, version, scripts (jaise npm start), aur un packages ki list hoti hai jinpar aapka project depend karta hai.
- Isme exact version ke bajay aksar ek version range likhi hoti hai, jisme Caret (^) ya Tilde (~) symbols ka use hota hai.

> package-lock.json
- Ye file tab automatic banti ya update hoti hai jab aap npm install chalate hain. Ye aapke project me installed har ek package (aur unke andar ki sub-dependencies) ka exact version, download URL, aur ek integrity hash (security verification) ka record rakhti hai.
- Isme koi symbol (^ ya ~) nahi hota, sirf exact version hota hai (e.g., "version": "4.18.2"). Ye ensure karti hai ki aapka dependency tree bilkul locked rahe.